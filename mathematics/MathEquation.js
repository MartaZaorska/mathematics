...
class MathEquation {
  ...
  getY(x) {
    ...
    //replace x
    let formula = ['(', ...this.#formula, ')'].map((item) =>
      item === 'x' ? x : item
    );
    ...
    while (closeBracketsIndex > 0) {
      ...
      const openBracketsIndex = formula
        .slice(0, closeBracketsIndex)
        .lastIndexOf('(');
      //get expression between the brackets
      const betweenBrackets = formula.slice(
        openBracketsIndex + 1,
        closeBracketsIndex
      );
  ...
  #createFormula = (formula) => {
    ...
    if (typeof formula !== 'string' || formula.length === 0)
      throw new Error('Invalid argument: formula must be a non-empty string.');
    ...
    if (invalidSign) {
      throw new Error(
        `Invalid mathematical formula: incorrect sign "${invalidSign}".`
      );
    }
  ...
  #findNumbers = (formulaArray) => {
    ...
    let startIndex = formula.findIndex(
      (item) => item.charCodeAt(0) >= 48 && item.charCodeAt(0) <= 57
    );

    //loop through all number signs
    while (startIndex >= 0) {
      let endIndex = formula
        .slice(startIndex)
        .findIndex((item) => FORMULA_SIGNS.indexOf(item) >= 0);

      if (endIndex < 0)
        throw new Error('Invalid mathematical formula: incorrect number.');
  ...
  #checkFormula = (formula) => {
    ...
    if (formula.length === 1) {
      ...
      throw new Error(
        'Invalid mathematical formula: single argument must be a number or "x".'
      );
    }
    ...
    if (FORMULA_RULES['start'].elements.indexOf(firstElement) < 0)
      throw new Error(
        `Invalid mathematical formula: incorrect first element "${firstElement}".`
      );

    //check last element
    const lastElement = this.#getEquationElement(formula[formula.length - 1]);
    if (FORMULA_RULES['end'].elements.indexOf(lastElement) < 0)
      throw new Error(
        `Invalid mathematical formula: incorrect last element "${lastElement}".`
      );
    ...
    for (let i = 0; i < formula.length - 1; i++) {
      ...
      if (FORMULA_RULES[element].elements.indexOf(nextElement) < 0)
        throw new Error(
          `Invalid mathematical formula: incorrect elements "${formula[i]}${
            formula[i + 1]
          }".`
        );
    }
  ...
  #checkBrackets = (formula) => {
    ...
    formula.forEach((item) => {
      ...
      if (brackets < 0)
        throw new Error('Invalid mathematical formula: incorrect brackets.');
    });
    ...
    while (operationIndex < operations.length) {
      ...
      const value1 =
        index === 0 || operation === 'r'
          ...
          : result[index - 1];
      const value2 =
        index === 0 || operation === 'r' ? undefined : result[index + 1];

      const value = this.#calculateExpression(operation, value1, value2);

      result = !value2
        ? replaceInArray(result, index, index + 1, value)
        : replaceInArray(result, index - 1, index + 1, value);
    }
  ...
  #calculateExpression = (operation, value1, value2 = undefined) => {
    switch (operation) {
      case 'r':
        if (value1 < 0)
          throw new Error(
            'Illegal math operations: the square root of negative numbers is not allowed.'
          );
      ...
      case '/':
        if (value2 === 0)
          throw new Error(
            'Illegal math operations: divided by 0 is not allowed.'
          );
        return value1 / value2;
      case '*':
        return value1 * value2;
      ...
      default:
        throw new Error(
          `Illegal math operations: "${operation}" operation does not exist`
        );
    }
  };
}
