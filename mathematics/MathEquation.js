...
class MathEquation {
  ...
  getY(x){
    ...
    if(typeof x !== 'number') throw new Error('Invalid argument: "x" must be a number');
  ...
  #createFormula = (formula) => {
    ...
    if(typeof formula !== 'string' || formula.length === 0) throw new Error("Invalid argument: formula must be a non-empty string.");
    ...
    if(invalidSign){
      throw new Error(`Invalid mathematical formula: incorrect sign "${invalidSign}".`);
    }
  ...
  #findNumbers = (formulaArray) => {
    ...
    while(startIndex >= 0){
      ...
      if(typeof number !== "number") throw new Error("Invalid mathematical formula: incorrect number.");
