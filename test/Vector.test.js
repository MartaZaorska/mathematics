...
test("Method createVector(): points A(-2,10,4.5) and B(1.3, -2, 0)", () => {
  expect(Vector.createVector([-2, 10,4.5], [1.3, -2, 0])).toStrictEqual([3.3,-12, -4.5]);
});
...
test("Method multiplyVector(): vector [1, 0, -0.5] and value 2.25", () => {
  expect(Vector.multiplyVector([1, 0, -0.5], 2.25)).toStrictEqual([2.25, 0, -1.125]);
});
...
test("Error: array of 5 elements", () => {
  ...
  }).toThrow("Invalid vector: the argument must be an array of 1, 2 or 3 elements.");
});
...
test("Error: a two-dimensional vector instead of a three-dimensional vector", () => {
  ...
  }).toThrow("Invalid argument: vectors must be an array containing three numbers.");
});
